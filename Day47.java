// You are given n numbers as well as n probabilities that sum up to 1.
// Write a function to generate one of the numbers with its corresponding probability.

// For example, given the numbers [1, 2, 3, 4] and probabilities [0.1, 0.5, 0.2, 0.2], your function should return 1 10% of the time,
// 2 50% of the time, and 3 and 4 20% of the time.

// You can generate random numbers between 0 and 1 uniformly.


public class Day47 
{
	
	public static int getRandomNum(int[] numbers, double[] probabilities)
	{
		double random = Math.random();
		double sum= 0;
		
		for(int i= 0; i<numbers.length; i++)
		{
		
			sum+=probabilities[i];
			
			// when found the number with the probability
			if(sum>= random)
				return numbers[i];
		}		
		
		return -1;
	}
	
	// Driver code
	public static void main(String args[])
	{
		
		int numbers[] = {1, 2, 3, 4};
		double probabilities[] = {0.1, 0.5, 0.2, 0.2};
		
		System.out.println(getRandomNum(numbers,probabilities));
	}
}